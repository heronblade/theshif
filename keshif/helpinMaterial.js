...
const measureLabelModeInfo = function () {
  const _p = 'percent (%) of the active records';
  const _a = 'absolute (#) value';
  const mode = this.browser.percentModeActive ? _p : _a;
  const mode_other = this.browser.percentModeActive ? _a : _p;
  const _total = this.browser.allRecordsAggr.recCnt.Active;
  const _active = Math.round(_total * 0.35); // fixed percentage. A good mid-range. Works just fine.
  const _filtered = this.browser.isFiltered() ? ' currently filtered ' : '';
  return `${'' +
...
const visualScaleModeInfo = function () {
  const _p = 'relative (%) active';
  const _a = 'absolute';
  const mode = this.browser.percentModeActive ? _p : _a;
  const mode_other = this.browser.percentModeActive ? _a : _p;
  return `${'' +
...
const percentileInfo = '<p>Each block shows a percentile range, such as 20%-30%. ' +
...
const measureMetricInfo = function () {
  let alternative = '';
...
const pickSelectedAggr = function () {
  const midP = this.records.length * 0.5;
  let minV = midP * 0.8,
    maxV = midP * 1.2;
  let curMax = this.records.length;
  let hAggr;
  this.allAggregates.some((aggr) => {
    ...
    const curDist = Math.abs(aggr.records.length - midP);
    if (curMax > curDist) {
      hAggr = aggr;
      curMax = curDist;
    }
...
const pickExplainAggr = function (sT) {
  let hAggr = null;
  let curMax = 1;
  this.allAggregates.some((aggr) => {
    if (aggr.emptyRecordsAggregate) return false;
    ...
    const _mActive = aggr._measure.Active;
    if (_mActive == 0) return false; // no active records. do not describe
    // TODO: skip global aggregate, etc etc.
    // Only keep ones that are categories or interval bins
    const curDist = aggr._measure[sT] / aggr._measure.Active;
...
const intervalSummaryInfoFunc = function (DOM) {
  const summary = DOM.__data__;
  const summaryName = summary.summaryName;
  const recordName = summary.browser.recordName;
  const _min = summary.printWithUnitName(summary.intervalRange.org.min);
  const _max = summary.printWithUnitName(summary.intervalRange.org.max);
...
const printBreadcrumb = function (sType, summary, aggr) {
...
const printBreadcrumb_Dummy = function (sType, _name, _value) {
  return `<span class='breadCrumb crumbMode_${sType}' ready='true'>` +
    '<span class=\'breadCrumbIcon fa\'></span>' +
    '<span class=\'crumbText\'>' +
...
const aggrDescription = function (aggr, encodingIcon) {
  const summary = aggr.summary;

  const recordName = this.browser.recordName;
  const aggrLabel = summary.printAggrSelection(aggr);
  const globalActive = browser.allRecordsAggr.recCnt.Active.toLocaleString();

  const _metric = getMetricText.call(this.browser);

  let str = '';

  const _temp = this.browser.percentModeActive;

  if (this.browser.ratioModeActive && this.browser.percentModeActive) {
    this.browser.percentModeActive = false;
  }

  const measureLabel = this.browser.getMeasureLabel(aggr, summary);
  ...
  ['Highlight', 'Compare_A', 'Compare_B', 'Compare_C'].forEach(function (sType) {
    if (this.browser.selectedAggr[sType]) {
      const _aggr = this.browser.selectedAggr[sType];
...
const _material = {
  _contextFeatures: {
    ...
    SummaryInBrowser: {
      ...
      v: { summaries(summary) { return summary.inBrowser(); } },
      weight: 10,
      fixBy: 'T_AddSummary',
    },
    CollapsedSummary: {
      descr: 'Summary is collapsed.',
      v: { summaries(summary) { return summary.collapsed; } },
      weight: 15,
    ...
    CategoricalSummary: {
      ...
      v: { summaries(summary) { return summary.type === 'categorical'; } },
      weight: 20,
      multiplier: 1.2,
    },
    CategoricalMultiValSummary: {
      descr: 'Summary attribute has multiple categorical values.',
      v: { summaries(summary) { return summary.type === 'categorical' && summary.isMultiValued; } },
      weight: 40,
    },
    IntervalSummary: {
      descr: 'Summary is interval (numeric or time).',
      v: { summaries(summary) { return summary.type === 'interval'; } },
      weight: 25,
    },
    NumberSummary: {
      descr: 'Summary attribute is numeric.',
      v: { summaries(summary) { return summary.type === 'interval' && summary.scaleType !== 'time'; } },
      weight: 30,
    },
    ...
    CategoricalMapSummary: {
      ...
      v: { summaries(summary) { return summary.catMap; } },
      weight: 40,
    },
    CategoricalMapVieweight: {
      descr: 'Summary is viewed as a categorical map.',
      v: { summaries(summary) { return summary.viewAs === 'map'; } },
      weight: 45,
    },
    MultiValuedSummary: {
      descr: 'Categorical summary includes multiple values per record.',
      v: { summaries(summary) { return summary.isMultiValued; } },
      weight: 40,
    },
    FilteredSummary: {
      descr: 'Summary is filtered.',
      v: { summaries(summary) { return summary.isFiltered(); } },
      weight: 50,
      fixBy: 'T_SelectFilter',
    },
    CategoricalSummaryWithTextSearch: {
      descr: 'Summary includes more than 20 categories.',
      v: { summaries(summary) { return summary.showTextSearch; } },
      weight: 30,
    },
    SummaryWithMissingValues: {
      descr: 'Summary has at least one invalid/missing record value.',
      v: { summaries(summary) { return summary.missingValueAggr.records.length > 0; } },
      weight: 47,
    },
    ExpandableSummary: {
      descr: 'Summary includes more categories than currently visible.',
      v: { summaries(summary) { return !summary.areAllCatsInDisplay(); } },
      weight: 20,
    },
    SortableSummary: {
      descr: 'Summary has more than 20 categories.',
      v: { summaries(summary) { return summary.configRowCount > 0 && !summary.catSortBy_Active.no_resort; } },
      weight: 22,
    },
    SummaryWithDescription: {
      descr: 'Summary has a description.',
      v: { summaries(summary) { return summary.description !== undefined; } },
      weight: 4,
    },
    SummaryOpenConfig: {
      descr: 'Summary configuration is open.',
      v: { summaries(summary) { return summary.DOM.root.attr('showConfig') === 'true'; } },
      weight: 50,
    ...
    NotAverageMeasure: {
      ...
      v() { return this.browser.measureFunc !== 'Avg'; },
      weight: 10,
    ...
    AnyFiltered: {
      ...
      v() { return this.browser.isFiltered(); },
      weight: 40,
      fixBy: 'T_SelectFilter',
    },
    ...
    AuthoringMode: {
      ...
      v() { return this.browser.authoringMode; },
      weight: 10,
      fixBy: 'T_EnableAuth',
    },
    LoggedIn: {
      descr: 'You need to be logged in.',
      v() { return kshf.gistLogin; },
      weight: 44,
      fixBy: 48, // login...
    },
  },
  ...
  _components: {
    ...
    Record: {
      matches: '.kshfRecord',
      info(DOM) {
        const r = DOM.__data__;
        const summary = this.browser.recordDisplay.sortingOpt_Active;
        const sortingSummaryName = summary.summaryName;
        const v = summary.getRecordValue(r);
        // TODO: if the record is highlighted (color), show why it is highlighted.
        const intro = `<p>This is a single record among ${this.browser.recordName}.</p>`;
        const ranking = `<p>It is ranked at <span class='bolder'>#${r.recordRank + 1}</span> with its ` +
        ...
        if (r.selectCompared_str) {
          ...
          r.selectCompared_str.replace(' ', '').split('').forEach(function (_char) {
            const comparedAggr = this.browser.selectedAggr[`Compare_${_char}`];
    ...
    'Percentile Chart': {
      ...
      info(DOM) {
        const summary = DOM.__data__;
        const summaryName = `<span class='bolder'>${summary.summaryName}</span>`;
        const recordName = this.browser.recordName;
        return `${'' +
    ...
    'Percentile Range': {
      ...
      info(DOM) {
        const recordName = this.browser.recordName;
        const perct_num = DOM.__data__;
        const summary = DOM.__data__.summary;
        const summaryName = summary.summaryName;
        var sType = DOM.parentNode.className.substr(16); // "Active", "Highlight", "Compare_A", etc...
        let minV = summary.quantile_val[sType + perct_num[0]];
        let maxV = summary.quantile_val[sType + perct_num[1]];

        let numRecords = 0;
        summary.filteredRecords.forEach((record) => {
          const v = summary.getRecordValue(record);
        ...
        if (sType === 'Compare_A' || sType === 'Compare_B' || sType === 'Compare_C') {
          const comparedAggr = this.browser.selectedAggr[sType];
    ...
    'Global Measurement': {
      ...
      pos: 'nw',
      info(DOM) {
        const measureLabel = `<span class='bolder'>${this.browser.getGlobalActiveMeasure()}</span>`;

        let filterSel = this.browser.DOM.breadcrumbs.selectAll('.crumbMode_Filter').nodes().length;
    ...
    Breadcrumbs: {
      matches: '.breadcrumbs',
      info(DOM) {
        const totalSel = this.browser.DOM.breadcrumbs.selectAll('.breadCrumb').nodes().length;
        let filterSel = this.browser.DOM.breadcrumbs.selectAll('.crumbMode_Filter').nodes().length;
        let compareSel = this.browser.DOM.breadcrumbs.selectAll('[class*="crumbMode_Compare_"]').nodes().length;
        if (filterSel === 0) filterSel = 'none';
  ...
  _topics: {
    T_SelectHighlight: {
      ...
      activate() {
        const hAggr = pickSelectedAggr.call(this.browser);
      ...
      animate: {
        ...
        4.5() {
          const _aggr = pickExplainAggr.call(this.browser, 'Highlight');
        ...
        6() {
          ...
          const minY = this.browser.recordDisplay.DOM.recordGroup.node().scrollTop;
          const maxY = minY + this.browser.recordDisplay.DOM.recordGroup.node().offsetHeight;
          this.browser.selectedAggr.Highlight.records.some(function (record) {
            if (!record.isWanted) return false;
            const DOM = record.DOM.record;
    ...
    T_SelectFilter: {
      ...
      animate: {
        ...
        // Show the other (explain) aggregate **************************************
        6.5() {
          const _aggr = pickExplainAggr.call(this.browser, 'Active');
          this.context.HighlightedDOM = [_aggr.DOM.aggrGlyph];
    ...
    T_SelectCompare: {
      ...
      animate: {
        ...
        // Show the other (explain) aggregate *********************************************
        6() {
          const cT = this.context.fakeCompare;
          const _aggr = pickExplainAggr.call(this.browser, `Compare_${cT}`);
          this.context.HighlightedDOM = [_aggr.DOM.aggrGlyph];
          // TODO: make the tooltip context dependent. - visual scale, metric, label
          this.fHighlightBox(
        ...
        9() {
          ...
          const _aggr = pickSelectedAggr.call(this.browser);
    ...
    T_FilterOr: {
      ...
      tAnswer: {
        ...
        activate() { this.context.HighlightedDOM[0].setAttribute('mouseOver', true); },
        deactivate() { this.context.HighlightedDOM[0].removeAttribute('mouseOver'); },
      },
    },
    T_FilterNot: {
    ...
    T_RecordTextSearch: {
      q() { return `Select ${this.browser.recordName} by text search`; },
      actions: 'Explore+Select',
    ...
    T_ExploreMissingVal: {
      q() { return `Explore ${this.browser.recordName} with missing/invalid values`; },
      actions: 'Explore+Select',
      topics: 'Aggregate',
    ...
    T_ChangeMeasureLabel: {
      q() {
        const text = this.browser.percentModeActive ? 'absolute (#)' : 'percent (%)';
      ...
      animate: {
        1() {
          this.context.HighlightedDOM[0].stencilBox.append('div')
            .attr('class', 'pointerAction').style('left', '-30px').append('span')
    ...
    T_ChangeVisualScale: {
      ...
      animate: {
        1() {
          const s = this.context.HighlightedDOM[0].parentNode.__data__;
          s.skipExplain = true;
        ...
        5() {
          const _temp = pickExplainAggr.call(this.browser, 'Highlight');
          this.context.HighlightedDOM = [_temp.DOM.aggrGlyph];
          const aggrName = `${_temp.summary.printAggrSelection(_temp)} ${_temp.summary.summaryName}`;
          const _active = _temp.recCnt.Active;
          const _highlight = _temp.recCnt.Highlight;
          this.fHighlightBox(
    ...
    T_GetRecordDetails: {
      ...
      tAnswer: {
        ...
        filter() {
          const minY = this.browser.recordDisplay.DOM.recordGroup.node().scrollTop;
          const maxY = minY + this.browser.recordDisplay.DOM.recordGroup.node().offsetHeight;
          this.context.HighlightedDOM = this.context.HighlightedDOM.filter((d) => {
            const record = d.__data__;
            if (!record.isWanted) return false;
            const DOM = record.DOM.record;
            if (typeof DOM === 'undefined') return false;
            if (DOM.offsetTop < minY) return false;
    ...
    T_ClearFilters: {
      q() { return `Clear all filters / Show all ${this.browser.recordName}`; },
    ...
    T_RecSortRev: {
      q() { return `Sort ${this.browser.recordName} in reverse`; },
    ...
    T_RecSortChange: {
      q() { return `Change sorting criteria of ${this.browser.recordName}`; },
      actions: 'Explore+Sort',
      topics: 'Record Panel',
...
if (location.search === '?exp=basis' || location.search === '?exp=helpin' || location.search === '?exp=train') {
  ...
  window.onbeforeunload = function (e) {
    const message = 'Please do not reload this page.';
    e = e || window.event;
    if (e) e.returnValue = message; // For IE and Firefox prior to version 4
    return message;
  };

  const isTraining = window.location.href !== 'http://localhost/keshif/demo/helpin_dc_homicides.html?exp=train';

  const timeLimit = isTraining ? (60 * 2.5 + 10) : (60 * 8 + 20);
